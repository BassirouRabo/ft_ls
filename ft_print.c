...
/*   Updated: 2018/01/04 23:55:36 by brabo-hi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		ft_print_l(t_liste *node, t_len *len, char *tmp)
{
	...
	{
		maj = ft_itoa(major(node->sbuf->st_rdev));
		min = ft_itoa(minor(node->sbuf->st_rdev));
		ft_printf("  %s, %s", ft_add_space_d(maj, len->maj),
				ft_add_space_d(min, len->min));
		ft_strdel(&maj);
		ft_strdel(&min);
	}
	else
	{
		size = ft_itoa(node->sbuf->st_size);
		ft_printf("  %s ", ft_add_space_d(size, len->size));
		ft_strdel(&size);
	}
	ft_printf(" %s ", tmp);
}
